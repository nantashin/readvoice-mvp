...
- **현재 버전:** v2.9.0
- **확정 모델 구성 변경 제안 금지**
- **오늘 작업:** P2(EXAONE→qwen3.5:4b) 진행 중
- **비개발자 사용자** — 계획 승인 후 구현
- **thinking mode 억제 필수** (/no_think)
- **납품 제외:** qwen/glm (중국산)
- **납품 허용:** solar/claude API
...
분류 결과별 TTS:
  [사진] "이미지로 판단했어요"
    → 모델 선택: 솔라(1.5~2분) / 구글포지(30~40초) / 라마비전(2~3분)
  [문서] "문서 이미지로 판단했어요"
    → 모델 선택: 솔라(1.5~2분) / 큐쓰리(30~60초) / 올름오씨알(1~2분) / 구글포지(30~40초)
  [혼합] "그림과 글자가 함께 있어요"
    → 선택: 그림 먼저 / 글자 먼저
...
### 이미지 분석 (5개 모델)
- **솔라 (solar:10.7b):** 한국산 Upstage (1.5~2분) - 일번
- **구글 사기가 (gemma4:e4b):** 정확하고 빠름 (1분) - 이번
- **큐쓰리 (qwen3.5:9b):** 가장 정확 (2분) - 삼번
- **구글 이기가 (gemma4:e2b):** 가장 빠름 (30초) - 사번
- **라마비전 (llama3.2-vision):** 상세 묘사 (1분 30초) - 오번
- 음성 명령: "일번~오번" 또는 모델명
- 실사용 추천: 솔라, 구글 사기가

### 문서 OCR (4개 모델)
- **솔라 (solar:10.7b):** 한국산 Upstage (1.5~2분) - 일번
- **큐쓰리 (qwen3.5:9b):** 추천, 범용 (30~60초) - 이번
- **올름오씨알 (olmocr2):** 표/레이아웃 특화 (1~2분) - 삼번
- **구글 포지 (gemma4:e4b):** 범용 fallback (30~40초) - 사번
- 음성 명령: "일번~사번" 또는 모델명
