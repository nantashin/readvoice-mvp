...
지금 되는 것만 설명하고, 앞으로 할 것은 솔직하게 표시합니다.

### 현재 테스트 중
- 음성 명령 인식률 (다시/멈춰/천천히/빠르게)
- 이미지 8종 유형별 분석 정확도
- 세션 타임아웃 없이 장시간 사용 가능 여부

### 계획 중 (미완료)
- NVDA (윈도우 스크린리더) 호환성 테스트
- 센스리더 (국내 대표 스크린리더) 호환성 테스트
- VoiceOver (iOS), TalkBack (Android) 테스트
- 키보드만으로 전체 기능 사용 가능 여부
- 실제 시각장애인 사용자 5인 테스트

### 자동화 테스트
- Playwright + axe-core 접근성 자동 검사 (추가 예정)
