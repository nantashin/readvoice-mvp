...
export const IMAGE_MODELS = [
  {
    id: "gemma4:e4b",
    label: "1번. 구글 4기가 (gemma4 E4B) — 정확하고 빠름 (1분)",
    tts: "일번. 구글 4기가. 정확하고 빠릅니다. 약 1분 걸립니다."
  },
  {
    id: "qwen3.5:9b-image",
  ...
  {
    id: "gemma4:e2b",
    label: "3번. 구글 2기가 (gemma4 E2B) — 빠른 분석 (30초)",
    tts: "삼번. 구글 2기가. 가장 빠릅니다. 약 30초 걸립니다."
  },
  {
...
export const DOCUMENT_MODELS = [
  {
    id: "gemma4:e4b",
    label: "1번. 구글 4기가 (gemma4 E4B) — 범용 문서 (1분)",
    tts: "일번. 구글 4기가. 범용 문서에 강합니다. 약 1분 걸립니다."
  },
  {
    id: "qwen3.5:9b",
    label: "2번. 큐쓰리 (qwen3.5:9b) — 텍스트 전용 (1~2분)",
    tts: "이번. 큐쓰리. 텍스트 전용입니다. 약 1분에서 2분 걸립니다."
  },
  {
    id: "gemma4:e2b",
    label: "3번. 구글 2기가 (gemma4 E2B) — 빠른 분석 (30초)",
  ...
  {
    id: "llama3.2-vision:11b-instruct-q4_K_M",
    label: "4번. 라마비전 (Llama Vision) — 정밀 분석 (1분 30초)",
    tts: "사번. 라마비전. 정밀 분석입니다. 약 1분 30초 걸립니다."
  },
  {
    id: "richardyoung/olmocr2:7b-q8",
    label: "5번. 올름오씨알 (olmOCR2) — 표/레이아웃 특화 (2분)",
    tts: "오번. 올름오씨알. 표와 레이아웃 특화입니다. 약 2분 걸립니다."
  }
]
...
interface FileUploadProps {
  ...
  autoOpen?: boolean  // 파일 선택 창 자동 열기
}

export default function FileUpload({ onResult, onStatusChange, selectedModel, onModelChange, onFileSelected, autoOpen }: FileUploadProps) {
  ...
  const processFile = async (file: File, uiModelId: string) => {
    ...
    if (modelSelectionTimerRef.current) {
      ...
      modelSelectionTimerRef.current = null
    }
    ...
    if (validation.isPDF) {
      ...
      try {
        ...
        if (res.ok && data.text) {
          ...
          const defaultModel = localStorage.getItem("defaultModel") || "gemma4:e4b"
          // UI ID를 실제 모델 ID로 변환
          const actualModelId = MODEL_ID_MAP[defaultModel] || defaultModel
          const pdfOcrModels = ["qwen3.5:9b", "richardyoung/olmocr2:7b-q8", "gemma4:e4b", "gemma4:e2b", "llama3.2-vision:11b-instruct-q4_K_M"]
          const useModel = pdfOcrModels.includes(actualModelId) ? actualModelId : "gemma4:e4b"

          const modelName = useModel === "qwen3.5:9b" ? "큐쓰리" :
                           useModel === "richardyoung/olmocr2:7b-q8" ? "올름오씨알" :
                           useModel === "gemma4:e4b" ? "구글 4기가" :
                           useModel === "gemma4:e2b" ? "구글 2기가" : "라마비전"

          console.log(`[PDF] 스캔된 문서 감지 - ${modelName} 모델로 실행`)
          onStatusChange("speaking")
          tts.speak(`스캔된 문서예요. ${modelName} 모델로 읽어드릴게요.`, 1.0, () => {
            setTimeout(async () => {
              try {
                onStatusChange("processing")
                bgmManager.start()
