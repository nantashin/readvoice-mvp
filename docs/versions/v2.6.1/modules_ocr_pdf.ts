  ...
  try {
    ...
    try {
      ...
      if (parsed.success && parsed.base64) {
        ...
        const callVisionModel = async (modelName: string): Promise<string | null> => {
          ...
          try {
            ...
            console.log(`[PDF] ${modelName} 응답 길이:`, data.message?.content?.length || 0, "자")
            ...
            if (text && text.length > 5) {
              ...
              console.log(`[PDF] ${modelName} 빈 응답 - fallback 필요`)
            ...
            const msg = e instanceof Error ? e.message : String(e)
            console.error(`[PDF] ${modelName} OCR 실패:`, msg)
            return null
          }
        }

        // Fallback 모델 체인
        const fallbackModels = ["gemma4:e2b", "gemma4:e4b", "qwen3.5:9b"]
        ...
        try {
          for (const model of modelsToTry) {
            console.log(`[PDF] 시도 중: ${model}`)
            finalText = await callVisionModel(model)

            if (finalText) {
              console.log(`[PDF] ${model} 성공!`)
              return `파일명: ${name}\n\n${prefix}${finalText}`
            } else {
              console.log(`[PDF] ${model} 실패, 다음 모델 시도...`)
            }
          }

          // 모든 모델 실패 → Tesseract 결과라도 반환
          if (tesseractDraft && tesseractDraft.length > 50) {
            console.log(`[PDF] 모든 Vision 모델 실패, Tesseract 결과 반환 (${tesseractDraft.length}자)`)
            return `파일명: ${name}\n\n${prefix}[Tesseract 초안]\n${tesseractDraft}`
          }
