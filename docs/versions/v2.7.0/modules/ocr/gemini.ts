...
1. 이미지 속 텍스트:
   - 제일 큰 글씨부터 읽기 (크기와 위치 언급)
   - 화면 중앙 상단에 제목이 있으면 먼저 읽기
   - 그 다음 왼쪽→오른쪽, 위→아래 순서로 읽기
   - <>, <<>>, [], {} 같은 기호는 빼고 안의 텍스트만 읽기 (제목이나 소제목일 확률 높음)
...
6. 이미지 요약: 중앙상단에 큰 제목이 있었다면 그 제목과 위 내용을 바탕으로 이 이미지가 무엇인지 한 문장으로 요약

주의사항:
- 이미지와 텍스트가 상하로 붙어있으면 함께 설명 (예: "사진 아래 '강아지'라는 텍스트")
...
1. 텍스트:
   - 제일 큰 글씨부터 읽기 (크기와 위치 언급: "화면 중앙 상단에 큰 글씨로...")
   - 화면 중앙 상단에 제목이 있으면 먼저 읽기
   - 그 다음 왼쪽→오른쪽, 위→아래 순서로 읽기
   - <>, <<>>, [], {} 같은 기호는 빼고 안의 텍스트만 읽기 (제목이나 소제목)
...
5. 이미지 요약: 중앙상단에 큰 제목이 있었다면 그 제목과 위 내용을 바탕으로 이 이미지가 무엇인지 한 문장으로 요약
...
- Start with the LARGEST text first (mention size and position: "center-top in large font...")
- Read center-top title first if present
...
If there was a large title at center-top, summarize what this image is about in one sentence based on that title and the content above.
