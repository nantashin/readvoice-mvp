...
**문제:**
- "인물관계에도" → "인물관계도" 매칭 실패
- 완전 포함 여부만 확인하여 유사 파일명 인식 불가

**해결:**
- 70% 유사도 기준 매칭 추가
- 입력의 각 글자가 파일명에 포함되는지 체크
- 기존 매칭 로직 유지 + 유사도 매칭 추가
...
### ✅ 파일 목록 한국어 읽기
**변경:**
- TTS 출력: "1번, 2번, 3번..." → "일번, 이번, 삼번..."
- 음성 인식과 일치 (사용자는 "일번"이라고 말함)
...
### [긴급] 발견된 버그 🐛
**1. Edge TTS tts.stop() 즉시 중단 미작동**
- 문제: 스페이스바 눌러도 TTS 계속 재생됨
- 원인: Edge TTS의 stop() 메서드 구현 필요
- 영향: Push-to-Talk 사용성 저하

**2. loadFileByName 후 모델 선택 미작동**
- 문제: 파일 선택 후 모델 선택 멘트 안 나옴
- 원인: 파악 필요
- 영향: 파일 분석 불가

**3. 이미지/문서 분석 전체 흐름 테스트 필요**
- 파일 선택 → 모델 선택 → 분석 실행 → 결과 재생
- 8종 이미지 타입별 프롬프트 테스트
- 문서 OCR 3개 모델 테스트
